...
# from cuml.ensemble import XGBRegressor
...
class RNNRegressor(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_layers=2):
        ...
        self.fc = nn.Linear(hidden_dim, 1)  # Fully connected layer to output the final prediction

    def forward(self, x):
        # Initialize hidden state, adjusting for batch size dynamically
        batch_size = x.size(0)  # Get the batch size from the input x

        # Initialize h0 with the correct dimensions (num_layers, batch_size, hidden_dim)
        h0 = torch.zeros(self.num_layers, batch_size, self.hidden_dim).to(x.device)
        h0 = h0.flatten(start_dim=1)

        print(h0.dim())

        # Forward propagate the input through the RNN layer
        out, _ = self.rnn(x, h0)  # out: tensor of shape (batch_size, seq_length, hidden_dim)

        # Pass the output of the last time step to the fully connected layer
        out = self.fc(out[:, -1, :])  # out: tensor of shape (batch_size, 1)
...
if __name__ == "__main__":
    ...
    while (starting + pd.DateOffset(years=11 + counter)) <= pd.to_datetime("20240101", format="%Y%m%d"):
        ...
        X_train = torch.cuda.FloatTensor(train[stock_vars].values)
        Y_train = torch.cuda.FloatTensor(train[ret_var].values).unsqueeze(1)
        X_val = torch.cuda.FloatTensor(validate[stock_vars].values)
        Y_val = torch.cuda.FloatTensor(validate[ret_var].values).unsqueeze(1)
        X_test = torch.cuda.FloatTensor(test[stock_vars].values)
    ...
    for model_name in ["rf", "cnn"]:    # "rnn", "lstm"]:
